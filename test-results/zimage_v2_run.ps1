...
$jobs = Get-Content "C:\Users\drama\Documents\DESPII\Karysm\test-results\zimage_v2_jobs.json" -Raw | ConvertFrom-Json

$negative = 'blurry, low quality, deformed, distorted face, extra fingers, bad anatomy, bad hands, cartoon, illustration, painting, oversaturated, plastic skin, white skin, light skin, watermark, text, logo, signature, multiple heads, mutated, deformed pupils'
...
for ($i = 0; $i -lt $jobs.Count; $i++) {
  ...
  $workflow = @{
    ...
    '9' = @{ class_type = 'SaveImage'; inputs = @{ images = @('8',0); filename_prefix = "karysm_v2_$($job.name)" } }
  }
  ...
  try {
    ...
    Write-Host "[$idx/$($jobs.Count)] [$($job.name)] QUEUE FAILED: $($_.Exception.Message)"
    continue
  }
  ...
  while ($true) {
    ...
    if ($hist.PSObject.Properties.Name -contains $promptId) {
        ...
        foreach ($n in $h.outputs.PSObject.Properties) {
          if ($n.Value.images) {
            foreach ($im in $n.Value.images) {
              ...
              $out = Join-Path $pngDir $im.filename
              ...
              if (Test-Path $webp) {
                ...
                $summary += [pscustomobject]@{ idx=$idx; name=$job.name; tag=$job.tag; elapsed=$elapsed; webp=$webp; size=$sz }
              }
            }
          }
        }
      ...
      if ($h.status.status_str -eq 'error') {
        Write-Host "[$idx/$($jobs.Count)] [$($job.name)] ERROR"
        break
      }
    }
    if (((Get-Date) - $jobStart).TotalSeconds -gt 300) {
      Write-Host "[$idx/$($jobs.Count)] [$($job.name)] TIMEOUT"
      break
    }
  }
}
...
$summary | Export-Csv -NoTypeInformation -Path 'C:\Users\drama\Documents\DESPII\Karysm\test-results\v2_summary.csv'
Write-Host ("=== DONE in {0:N1}s ({1:N0} min) — {2} images ===" -f $totalElapsed, ($totalElapsed/60), $summary.Count)
