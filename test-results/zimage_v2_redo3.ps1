...
$jobs = Get-Content "C:\Users\drama\Documents\DESPII\Karysm\test-results\zimage_v2_redo3.json" -Raw | ConvertFrom-Json
$negative = 'nude, naked, topless, shirtless, bare chest, bare shoulders, exposed chest, exposed cleavage, cleavage, breasts, nipples, lingerie, underwear, undressed, no clothes, no shirt, strapless, towel wrap, towel only, nsfw, suggestive, sexual, blurry, low quality, deformed, distorted face, extra fingers, bad anatomy, bad hands, cartoon, illustration, painting, oversaturated, plastic skin, white skin, light skin, watermark, text, logo, signature, multiple heads, mutated, deformed pupils'
...
for ($i = 0; $i -lt $jobs.Count; $i++) {
  ...
  $workflow = @{
    ...
    '9' = @{ class_type = 'SaveImage'; inputs = @{ images = @('8',0); filename_prefix = "karysm_v2redo3_$($job.name)" } }
  }
        ...
        foreach ($n in $h.outputs.PSObject.Properties) {
          if ($n.Value.images) {
            foreach ($im in $n.Value.images) {
              ...
              $out = Join-Path $pngDir "karysm_v2_$($job.name)_v3.png"
              ...
              Write-Host ("[$idx/$($jobs.Count)] [$($job.name)] {0:N1}s -> {1}" -f $elapsed, (Split-Path $webp -Leaf))
            }
          }
        }
        break
