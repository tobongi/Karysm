...
const TIPS = [
  { icon: '☀️', title: 'Bonne lumière', desc: 'Lumière naturelle, face à une fenêtre' },
  { icon: '📸', title: 'Face à la caméra', desc: 'Visage droit, yeux ouverts' },
  { icon: '🧼', title: 'Peau propre', desc: 'Sans maquillage ni crème' },
];

export default function SkinCaptureScreen() {
  const [photo, setPhoto] = useState<string | null>(null); // base64
  const [photoUri, setPhotoUri] = useState<string | null>(null); // display URI
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <Text style={styles.subtitle}>
          Prenez un selfie pour analyser votre type de peau, carnation, et recevoir des recommandations personnalisées.
        </Text>
        ...
        {photo && (
          ...
          >
            {analyzing ? (
              ...
              <Text style={styles.analyzeText}>✨ Analyser ma peau</Text>
            )}
          </Pressable>
        )}
...
const styles = StyleSheet.create({
  ...
  preview: { width: 240, height: 240, borderRadius: 120, marginBottom: 12 },
  retakeButton: {
  ...
  analyzingHint: { fontSize: 13, color: colors.textMuted, textAlign: 'center', marginTop: 12 },
});
